...
struct RichTextEditor: NSViewRepresentable {
  ...
  final class Coordinator: NSObject, NSTextViewDelegate {
    ...
    func textView(
      ...
      let paraStart = ns.paragraphRange(for: NSRange(location: affectedRange.location, length: 0)).location
      let leadRange = NSRange(location: paraStart, length: affectedRange.location - paraStart)
      guard leadRange.length >= 0, NoteList.shouldConvert(typedLineStart: ns.substring(with: leadRange))
      else { return true }
      // Remplace le « - » tapé par « –\t » et pose le retrait suspendu ; l'espace qui a déclenché
      // la conversion est avalé (`return false`).
      guard textView.shouldChangeText(in: leadRange, replacementString: NoteList.prefix) else { return false }
      textView.textStorage?.replaceCharacters(in: leadRange, with: NSAttributedString(string: NoteList.prefix, attributes: listAttrs))
      textView.didChangeText()
      textView.setSelectedRange(NSRange(location: paraStart + (NoteList.prefix as NSString).length, length: 0))
    ...
    private var listAttrs: [NSAttributedString.Key: Any] {
      [.font: parent.font, .foregroundColor: parent.textColor, .paragraphStyle: NoteList.paragraphStyle()]
    }
    ...
    private func handleListNewline(_ textView: NSTextView) -> Bool {
      ...
      let plainAttrs: [NSAttributedString.Key: Any] = [.font: parent.font, .foregroundColor: parent.textColor]

      if NoteList.isEmptyItem(paragraph) {
        let stripRange = NSRange(location: paraRange.location, length: (NoteList.prefix as NSString).length)
        guard textView.shouldChangeText(in: stripRange, replacementString: "") else { return true }
        textView.textStorage?.replaceCharacters(in: stripRange, with: NSAttributedString(string: "", attributes: plainAttrs))
        textView.didChangeText()
      ...
      textView.textStorage?.replaceCharacters(in: sel, with: NSAttributedString(string: insertion, attributes: listAttrs))
      textView.didChangeText()
